...
const getJobDetailsDirect = async page => {
  ...
  const values = await Promise.all(
    keys.map(key => getJobDetailDirect(JOB_DETAIL_DIRECT_PROPERTIES[key]))
  );
...
const scrapeJobs = async postingModel => {
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    // Navigate the page to a URL
    await page.goto(
      `${BASE_URL}/jobsearch?${JOB_POSTING_AGE_FILTER}&page=${i}&${JOB_POSTING_SORT}&${JOB_POSTING_SOURCE_FILTER}`
    );
    ...
    for (let j = 0; j < jobIdsThisPage.length; j++) {
      ...
      const dbResult = await postingModel
        .findOne({ jobPageId: jobIdsThisPage[j] })
        .exec();
      if (!dbResult) {
        // Navigate the page to a URL
        ...
        try {
          ...
          try {
            const employmentTypeSelector = await page.waitForSelector(
              '[property="employmentType"]',
              { timeout: SHORT_TIMEOUT }
            );
            const employmentTypeValue = await employmentTypeSelector?.evaluate(
              el => el.childNodes[0].textContent
            );
            const employmentSubTypeValue =
              await employmentTypeSelector?.evaluate(
            ...
            console.log(
              `Skipping employmentType and employmentSubType due to error: ${err}.`
            );
          ...
          try {
            const startTimeSelector = await page.waitForSelector(
              '#tp_startDateId',
              { timeout: SHORT_TIMEOUT }
            );
            const startTime = await startTimeSelector?.evaluate(
              el => el.parentNode.childNodes[4].textContent
            );
          ...
          try {
            const benefitsSelector = await page.waitForSelector('.fa-gift', {
              timeout: SHORT_TIMEOUT,
            });
            const benefits = await benefitsSelector?.evaluate(
              el => el.textContent
            );
            const trimmedBenefits = benefits.trim();
            directJobDetails['benefits'] =
              trimmedBenefits === '' ? null : trimmedBenefits;
          } catch (err) {
          ...
          try {
            const vacanciesSelector = await page.waitForSelector(
              '#tp_vacancyNumber',
              { timeout: SHORT_TIMEOUT }
            );
            const vacancies = await vacanciesSelector?.evaluate(
              el => el.parentNode.childNodes[4].textContent
            );
            directJobDetails['vacancies'] = vacancies
              ? vacancies.split(' ')[0]
              : null;
          } catch (err) {
          ...
          try {
            const verifiedSelector = await page.waitForSelector('.job-marker', {
              timeout: SHORT_TIMEOUT,
            });
            const verified = await verifiedSelector?.evaluate(
              el => el.textContent.trim().toLowerCase() === 'verified'
            );
          ...
          try {
            ...
            directJobDetails['validThrough'] = new Date(validThrough.trim());
          } catch (err) {
            console.log(`Skipping description due to error: ${err}.`);
            directJobDetails['validThrough'] = null;
          }
        ...
        console.log(
          `Skipping job with id ${jobIdsThisPage[j]} due to duplicate id.`
        );
      }
    }
